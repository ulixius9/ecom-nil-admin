...
router.post('/',[
    ...
    check('customer.phone', 'Please enter a valid Phone Number').isMobilePhone(),
    check('customer.addresses[0].address1', 'Please enter a Address').not().isEmpty(),
    check('customer.addresses[0].phone', 'Please enter a valid Phone Number').isMobilePhone(),    
    check('customer.addresses[0].city', 'Please provide a City').not().isEmpty(),
    check('customer.addresses[0].country', 'Please enter a Country name').not().isEmpty(),
    check('customer.addresses[0].zip', 'Please enter a valid Zip Code').isPostalCode('IN')
] ,async(req, res) => {
    ...
    // Dealing With DB
    const {customer: { first_name, last_name, email, verified_email ,addresses, phone }} = req.body;
    const newCustomer = {
...
router.put('/customers/:id',[
    ...
    // Dealing With DB
    const {customer: { first_name, last_name, email, addresses, phone, accepts_marketing, tags, note }} = req.body;
    ...
    try{
        let customer = await Customer.findByIdAndUpdate(req.params.id, updatedCustomer);
        customer = await customer.save();
        res.status(200).json(customer);
    }catch(e){
        res.status(500).json({
